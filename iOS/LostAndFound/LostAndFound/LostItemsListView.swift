...
struct LostItemsListView: View {
    ...
    private var filteredReports: [Report] {
        reports
            .filter { $0.type == .lost && $0.status == .approved }
            .filter { report in
    ...
    private var infoBanner: some View {
        Text("Before you report the item you found, check the list to see if it has already been reported as lost.")
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text(reports.isEmpty ? "No lost items yet" : "No matching items")
                .font(.custom("IBMPlexSans", size: 20))
                .foregroundColor(ColorPalette.labelPrimary)
            
            Text(reports.isEmpty ? "Lost items will appear here once reported." : "Try adjusting your search or category filters.")
    ...
    private func loadReports() async {
        ...
        do {
            let fetchedReports = try await reportService.fetchApprovedLostReports()
